    ...
    Statement = [
      {
        ...
        Resource = data.terraform_remote_state.messaging.outputs.sqs_queue_arns["video-uploaded-event"]
        Condition = {
          ArnEquals = {
...
resource "aws_s3_bucket_notification" "start_process_trigger" {
  ...
  queue {
    queue_arn = data.terraform_remote_state.messaging.outputs.sqs_queue_arns["video-uploaded-event"]
